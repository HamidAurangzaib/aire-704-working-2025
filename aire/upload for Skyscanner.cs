...
namespace aire
{
    public partial class upload_for_Skyscanner : Form
    {
        public upload_for_Skyscanner()
        {
            InitializeComponent();
        }
        ado d = new ado();
        private void upload_for_Skyscanner_Load(object sender, EventArgs e)
        {
            d.connecter();
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            if (ope.ShowDialog() == DialogResult.OK)
            {
                ...
                OleDbCommand cmd = new OleDbCommand("select * from [Skys$]", con);
                con.Open();
                dt = new DataTable();
                ...
                d.cmdd = new SqlCommand("delete from interimsky", d.cn);
                ...
                await Task.Run(() =>
                {
                    ...
                    {
                        j++;
                        sql += " insert into interimsky values ('" + dt.Rows[i]["FROM"].ToString() + "','"
            ...
            d.cmdd = new SqlCommand("update interimsky set Montant='0' where (Montant='')", d.cn);
            d.cmdd.ExecuteNonQuery();
            label2.Visible = false;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (radioButton1.Checked || radioButton2.Checked)
            {
                ...
                if (radioButton1.Checked)
                {
                    ...
                    d.cmdd = new SqlCommand("delete interimcmprsky", d.cn);
                    d.cmdd.ExecuteNonQuery();
                    d.cmdd = new SqlCommand("exec cmprinterimsky", d.cn);
                    d.cmdd.ExecuteNonQuery();

                    d.cmdd = new SqlCommand("update interimcmprsky set Difference=(New_Price-Old_Price) where (Difference=0 AND Old_Price>0)", d.cn);
                    d.cmdd.ExecuteNonQuery();
                    d.cmdd = new SqlCommand("exec doblerowsinterimsky", d.cn);
                    d.cmdd.ExecuteNonQuery();
                }
                else if (radioButton2.Checked)
                {
                    d.cmdd = new SqlCommand("delete interimcmprsky", d.cn);
                    d.cmdd.ExecuteNonQuery();
                    d.cmdd = new SqlCommand("exec cmprinterimoldsky", d.cn);
                    d.cmdd.ExecuteNonQuery();
