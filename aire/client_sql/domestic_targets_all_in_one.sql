-- ============================================================
-- Domestic target prices + "new deal" fix
-- Database: DB_A61545_andycom
--
-- Run this whole file ONCE, as a single script.
--
-- There are no GO separators in this file, so it works in any
-- SQL Server tool. Each stored procedure is applied through
-- sp_executesql, because several ALTER PROCEDURE statements are
-- not allowed to share one batch.
-- ============================================================
...
-- ------------------------------------------------------------
SET @sql = N'ALTER PROCEDURE [dbo].[UpdateIsFoundStatusForGFDomesticAirline]
AS
BEGIN
    -- Step 1: IsTargetFound / IsOldTarget; reset month + deal flags
    UPDATE c
...
-- ------------------------------------------------------------
SET @sql = N'ALTER PROC [dbo].[serchWithoutFromToGOOGleDomestic]
...
-- ------------------------------------------------------------
SET @sql = N'ALTER PROC [dbo].[serchFromMultiGroupCityGOOGleDomesticEverywhere]
...
-- ------------------------------------------------------------
SET @sql = N'ALTER PROC [dbo].[serchToMultiGroupCityGOOGleDomesticEverywhere]
...
-- ------------------------------------------------------------
SET @sql = N'ALTER PROC [dbo].[serchFromToMultiGroupCityGOOGleDomesticEverywhere]
@From NVARCHAR(MAX) = '''', @To NVARCHAR(MAX) = '''', @IsTargetOnly bit,
@Airline NVARCHAR(100) = '''',
@Aircode NVARCHAR(50) = '''',
@Days NVARCHAR(10) = '''',
